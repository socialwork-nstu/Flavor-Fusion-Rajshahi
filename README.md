...
<head>
    ...
    <title>Flavor-Fusion Rajshahi | অর্ডার ও ছাড়</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: system-ui, 'Segoe UI', sans-serif; }
        ...
        .logo-img { width: 45px; height: 45px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px; box-shadow: 0 2px 8px rgba(0,0,0,0.2); }
        .logo-text h2 { font-size: 1.1rem; }
        .logo-text p { font-size: 0.7rem; opacity: 0.9; }
        .nav-buttons { display: flex; gap: 8px; }
        .nav-btn, .cart-icon { background: #2c3e2f; color: white; border: none; padding: 8px 14px; border-radius: 40px; cursor: pointer; font-size: 12px; transition: 0.3s; }
        ...
        .confirm-btn { background: #2c6e2f; width: 100%; padding: 14px; border: none; color: white; border-radius: 60px; font-weight: bold; font-size: 1.1rem; margin-top: 12px; cursor: pointer; }
...
<body>
<div class="navbar">
    <div class="logo-area"><div class="logo-img"><img src="https://i.ibb.co.com/twPGCqvF/FB-IMG-1778220649651-2.jpg" alt="Logo" style="width: 45px; height: 45px; border-radius: 50%; object-fit: cover;"></div><div class="logo-text"><h2>Flavor-Fusion Rajshahi</h2><p>খামার থেকে বাড়ি</p></div></div>
    <div class="nav-buttons"><button class="nav-btn" id="myOrdersBtn">📋 আমার অর্ডার</button><div class="cart-icon" id="viewCartBtn">🛒 <span id="cartCount">0</span></div></div>
</div>
...
function getDiscount(productId, kg) {
    ...
    for (let [rule, desc] of Object.entries(CUSTOM_DISCOUNTS)) {
...
if (Object.keys(allReviews).length === 0) allReviews = { 1: [{ name: "রাজিব", rating: 5, comment: "অসাধারণ আম।যথেষ্ট পাকা ছিলো আম। অর্ডার ঠিকঠাক পেয়েছি। আপনারা নিতে পারেন।", date: new Date().toLocaleString() }] };
...
function updateCartUI() { let c = cart.reduce((s,i)=>s+i.kg,0); document.getElementById('cartCount').innerText = c.toFixed(1); }
// স্লাইডার
const sliderItems = [
    { text: "🎉 ১০% ছাড়", subtext: "১২ কেজির উপরে অর্ডারে ছাড় পাবেন", icon: "🎉", bgImage: "linear-gradient(135deg,#ff6b35,#ff8c42)" },
    { text: "💰 বেশি কিনুন", subtext: "কম দামে পণ্য কিনুন", icon: "💰", bgImage: "linear-gradient(135deg,#e55a2b,#ff7e5e)" },
...
function updateSlider() {
    ...
    cont.offsetHeight;
    cont.style.animation = `${animDir ? 'slideInRight' : 'slideInLeft'} 0.6s ease`;
...
function renderProducts() {
    ...
    document.getElementById('productList').innerHTML = filtered.map(p => {
        ...
        return `<div class="product-card"><img src="${p.img}"><h3>${p.name}</h3><div class="price">৳${p.price}/কেজি</div>${badges}<div class="review-trigger" onclick="openReviewModal(${p.id})">⭐ রিভিউ দেখুন/লিখুন</div><div class="card-buttons"><button class="btn-add" onclick="addToCart(${p.id})">🛒 কার্টে যোগ</button><button class="btn-buy" onclick="startOrder(${p.id})">⚡ অর্ডার করুন </button></div></div>`;
    }).join('');
}
// কার্টে যোগ
window.addToCart = function(pid) {
    ...
    modalDiv.innerHTML = `<div class="modal-container"><div class="modal-header"><h3>কার্টে যোগ</h3><span class="close-modal" onclick="this.closest('.modal').remove()">&times;</span></div><div style="text-align:center"><h3>${p.name}</h3><p>৳${p.price}/কেজি</p><div style="display:flex;gap:15px;margin:20px 0"><div class="order-option" data-type="campus" style="flex:1;padding:10px;background:#ffe0b5;border-radius:24px;cursor:pointer">ক্যাম্পাস (৩কেজি)</div><div class="order-option" data-type="nationwide" style="flex:1;padding:10px;background:#ffe0b5;border-radius:24px;cursor:pointer">দেশব্যাপী (১২কেজি)</div></div><div class="quantity-input"><button id="cm">−</button><span id="ckg">3</span><button id="cp">+</button></div><div id="cartPriceBreak"></div><button class="confirm-btn" id="cartConfirmBtn">✅ কার্টে যোগ</button></div></div>`;
    document.body.appendChild(modalDiv);
    let upd = () => {
        let disc = getDiscount(p.id, kg);
        let orig = p.price * kg;
        let final = orig * (1 - disc.percent/100);
...
function showCart() {
    if(!cart.length) { document.getElementById('cartContent').innerHTML = '<div class="empty-cart">🛒 কার্ট খালি</div>'; }
        ...
        let total = 0;
        let html = cart.map((it,i)=> { total += it.total; return `<div class="cart-item"><div><strong>${it.name}</strong> (${it.kg} কেজি, ${it.type==='campus'?'ক্যাম্পাস':'ডেলিভারি'})<br>মূল্য: ৳${it.total} ${it.discount?`(${it.discount}% ছাড়)`:''}</div><button class="cart-remove" onclick="removeFromCart(${i})">🗑️</button></div>`; }).join('');
...
function checkoutCart() { if(!cart.length){ showToast('কার্ট খালি'); return; } alert('কার্ট থেকে অর্ডার সম্পূর্ণ করতে শীঘ্রই আপডেট আসছে'); }
// অর্ডার প্রক্রিয়া
let currentProduct = null, currentOrderType = null, currentMinKg = 3;
function startOrder(pid) { currentProduct = products.find(p=>p.id===pid); showOrderOptions(); }
...
function showOrderForm() {
    ...
    <div class="form-group"><input type="text" id="orderName" placeholder="👤 নাম"></div>
    <div class="form-group"><input type="tel" id="orderPhone" placeholder="📞 মোবাইল"></div>
    <div class="form-group"><textarea id="orderAddress" rows="2" placeholder="🏠 ঠিকানা"></textarea></div>
    ...
    function updatePrice() {
        ...
        let fin = orig - save;
        document.getElementById('priceBreak').innerHTML = `<div>মূল: ৳${orig.toFixed(2)}</div>${d.percent ? `<div style="color:#e74c3c">মূল্য ছাড় (${d.percent}%): -৳${save.toFixed(2)}</div><div><strong> মূল্য: <span class="saved-amount">৳${fin.toFixed(2)}</span></strong></div><div>আপনি বাঁচাচ্ছেন: ৳${save.toFixed(2)}</div>` : `<div><strong>মোট: ৳${fin.toFixed(2)}</strong></div>`}`;
...
window.submitFinalOrder = async function() {
    ...
    let trx = '';
    if(pay === 'bkash') { trx = document.getElementById('trxId')?.value.trim(); if(!trx){ showToast('TrxID দিন'); return; } } else trx = 'COD';
    if(!name || !phone || !addr) { showToast('সব তথ্য দিন'); return; }
    let disc = getDiscount(currentProduct.id, kg);
    let finalTotal = currentProduct.price * kg * (1 - disc.percent/100);
    let orderData = { id: Date.now(), name, phone, address: addr, items: [{ name: currentProduct.name, kg, pricePerKg: currentProduct.price, total: finalTotal, discount: disc.percent }], total: finalTotal, paymentMethod: pay, trxId: trx, date: new Date().toLocaleString() };
    await fetch("https://script.google.com/macros/s/AKfycbxRWUVfuinB7ZMbQLUI5369QCYvLL-7guMdbE-IjioPzvhfTDUF_yaUvK22dtcEwjf1/exec", { method:'POST', mode:'no-cors', headers:{'Content-Type':'application/json'}, body: JSON.stringify(orderData) });
    allOrders.unshift(orderData); saveOrders();
    document.getElementById('confirmationContent').innerHTML = `<div class="confirmation-message"><div class="success-icon">✅</div><h2>আপনার অর্ডার কনফার্ম হয়েছে!শীগ্রই আমরা আপনার সাথে যোগাযোগ করবো</h2><div>#${orderData.id}</div><div>নাম: ${name}</div><div>মোট: ৳${finalTotal}</div><button class="confirm-btn" onclick="closeAllModals()">ঠিক আছে</button></div>`;
...
document.getElementById('verifyPhoneBtn').onclick = () => {
    ...
    let html = orders.length ? orders.map(o=>`<div class="order-card"><div><strong>#${o.id}</strong> <small>${o.date}</small></div><div>📦 ${o.items.map(i=>`${i.name} (${i.kg} কেজি) ${i.discount?`- ${i.discount}% ছাড়` : ''}`).join(', ')}</div><div>💰 ৳${o.total}</div><div>🔑 ${o.trxId || 'COD'}</div></div>`).join('') : '<div class="empty-cart">কোনো অর্ডার নেই</div>';
